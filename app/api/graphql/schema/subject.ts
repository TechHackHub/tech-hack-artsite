...
builder.mutationField('createSubject', (t) => t.prismaField({
  type: 'Subject',
...
builder.mutationField('updateSubject', (t) => t.prismaField({
  type: 'Subject',
  authScopes: { isLogin: true },
  args: {
...
builder.mutationField('deleteSubject', (t) => t.prismaField({
  type: 'Subject',
  authScopes: { isLogin: true },
  ...
  resolve: async (query, _, args) => {
    ...
    if (containArtworks) {
      throw new Error('Cannot be deleted, the subject data is used by artworks');
    }
