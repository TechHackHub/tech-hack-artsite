...
builder.mutationField('createMaterial', (t) => t.prismaField({
  type: 'Material',
  args: {
...
builder.mutationField('updateMaterial', (t) => t.prismaField({
  type: 'Material',
  args: {
...
builder.mutationField('deleteMaterial', (t) => t.prismaField({
  type: 'Material',
  authScopes: { isLogin: true },
  ...
  resolve: async (query, _, args) => {
    ...
    if (containArtworks) {
      throw new Error('Cannot be deleted, the material data is used by artworks');
    }
