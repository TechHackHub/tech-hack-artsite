...
builder.queryField('getArtwork', (t) => t.prismaField({
  ...
  resolve: async (_query, _, args) => await prisma.artwork.findUnique({
    where: { id: args.id }
  })
}));
...
builder.mutationField('createArtwork', (t) => t.prismaField({
  type: 'Artwork',
...
builder.mutationField('updateArtwork', (t) => t.prismaField({
  type: 'Artwork',
  authScopes: { isLogin: true },
  args: {
  ...
  resolve: async (query, _, args) => {
    ...
    return await prisma.artwork.update({
      ...
      data: {
        ...
        completedAt: input?.completedAt ? new Date(input.completedAt) : artwork?.completedAt,
...
builder.mutationField('publishArtworks', (t) => t.int({
  authScopes: { isLogin: true },
  args: {
...
builder.mutationField('deleteArtwork', (t) => t.prismaField({
  type: 'Artwork',
  authScopes: { isLogin: true },
